...
type gramaticaListener interface {
	...
	// EnterStructInst is called when entering the structInst production.
	EnterStructInst(c *StructInstContext)

	// EnterStructInit is called when entering the structInit production.
	EnterStructInit(c *StructInitContext)

	// EnterListaStructs is called when entering the listaStructs production.
	EnterListaStructs(c *ListaStructsContext)

	// EnterAccesoStruct is called when entering the accesoStruct production.
	EnterAccesoStruct(c *AccesoStructContext)

	// EnterAsigStruct is called when entering the asigStruct production.
	EnterAsigStruct(c *AsigStructContext)

	// EnterFunStruct is called when entering the funStruct production.
	EnterFunStruct(c *FunStructContext)

	// EnterCaja is called when entering the caja production.
	EnterCaja(c *CajaContext)

	// EnterValores is called when entering the valores production.
	EnterValores(c *ValoresContext)

	// EnterActStruct is called when entering the actStruct production.
	EnterActStruct(c *ActStructContext)
	...
	// ExitStructInst is called when exiting the structInst production.
	ExitStructInst(c *StructInstContext)

	// ExitStructInit is called when exiting the structInit production.
	ExitStructInit(c *StructInitContext)

	// ExitListaStructs is called when exiting the listaStructs production.
	ExitListaStructs(c *ListaStructsContext)

	// ExitAccesoStruct is called when exiting the accesoStruct production.
	ExitAccesoStruct(c *AccesoStructContext)

	// ExitAsigStruct is called when exiting the asigStruct production.
	ExitAsigStruct(c *AsigStructContext)

	// ExitFunStruct is called when exiting the funStruct production.
	ExitFunStruct(c *FunStructContext)

	// ExitCaja is called when exiting the caja production.
	ExitCaja(c *CajaContext)

	// ExitValores is called when exiting the valores production.
	ExitValores(c *ValoresContext)

	// ExitActStruct is called when exiting the actStruct production.
	ExitActStruct(c *ActStructContext)
